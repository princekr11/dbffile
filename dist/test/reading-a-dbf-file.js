...
describe('Reading a DBF file', () => {
    let tests = [
        ...
        },
        {
            ...
            recordCount: 45,
            dateOfLastUpdate: new Date('2014-04-14'),
            firstRecord: { [dbffile_1.DELETED]: true, AFCLPD: 'W', AFHRPW: 0, AFACCL: 'P', AFCRDA: new Date('1991-04-15'), AFPSDS: '' },
            lastRecord: { AFCLPD: 'W', AFHRPW: 0, AFLVCL: 0.00, AFCRDA: new Date('1991-04-15'), AFPSDS: '' },
            deletedCount: 0,
        },
        {
    ...
    tests.forEach(test => {
        it(`readRecords: ${test.description}`, async () => {
            ...
            let records;
            try {
                dbf = await dbffile_1.DBFFile.open(filepath, options);
                records = await dbf.readRecords(numberOfRecordsToRead);
            }
            catch (err) {
                chai_1.expect(err.message).contains(expectedError !== null && expectedError !== void 0 ? expectedError : '??????');
                return;
            }
            chai_1.expect(dbf.recordCount, 'the record count should match').equals(expectedRecordCount);
            chai_1.expect(dbf.dateOfLastUpdate, 'the date of last update should match').deep.equals(expectedDateOfLastUpdate);
            chai_1.expect(records[0], 'first record should match').to.deep.include(expectedFirstRecord);
            chai_1.expect(records[0][dbffile_1.DELETED], 'first record should match').equals(expectedFirstRecord[dbffile_1.DELETED]);
            chai_1.expect(records[records.length - 1], 'last record should match').to.deep.include(expectedLastRecord);
            chai_1.expect(records[records.length - 1][dbffile_1.DELETED], 'last record should match').equals(expectedLastRecord[dbffile_1.DELETED]);
        ...
        it(`asyncIterator: ${test.description}`, async () => {
            ...
            try {
                ...
                for await (const record of dbf) {
                    records.push(record);
                    if (numberOfRecordsToRead !== undefined && records.length >= numberOfRecordsToRead)
                        break;
                }
            }
            catch (err) {
